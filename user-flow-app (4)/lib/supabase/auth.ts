...
export async function signUp(email: string, password: string, role: Role = "passenger", name: string) {
  ...
  if (data.user) {
    ...
    if (role === "conductor") {
      ...
      const { error: conductorError } = await supabase.from("conductors").insert({
